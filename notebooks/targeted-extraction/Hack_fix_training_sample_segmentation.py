...
duplicates_stream = [
    item
    for item, count in collections.Counter([d["text"] for d in stream_list]).items()
    if count > 1
]
...
duplicates_segmented_stream = [
    item
    for item, count in collections.Counter(
        [d["text"] for d in segmented_stream_list]
    ).items()
    if count > 1
]
...
duplicates_at = [
    item for item, count in collections.Counter(stream_at_texts).items() if count > 1
]
len(duplicates_at)
# 48
duplicates_rh = [
    item for item, count in collections.Counter(stream_rh_texts).items() if count > 1
]
len(duplicates_rh)
# 4
duplicates_rh_at = [
    item for item, count in collections.Counter(stream_at_rh_texts).items() if count > 1
]
...
for d in duplicates_rh_at:
    dup_list.append(
        {k: v for k, v in d.items() if k in ["text", "spans", "_annotator_id"]}
    )
len(dup_list)
...
def remove_dupes(ll: List[dict], k: str) -> List[dict]:
...
[
    item
    for item, count in collections.Counter(
        [d["text"] for d in stream_at_rh_list_dedup]
    ).items()
    if count > 1
]
