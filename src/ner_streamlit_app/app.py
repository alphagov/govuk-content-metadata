...
with st.sidebar:
    ...
    st.metric(label="Average F-Score", value=np.round(np.mean([metrics_1["model_f"],metrics_2["model_f"]]),3), delta="1st")
    st.metric(label="Average Precision", value=np.round(np.mean([metrics_1["model_p"],metrics_2["model_p"]]),3), delta="1st")
    st.metric(label="Average Recall", value=np.round(np.mean([metrics_1["model_r"],metrics_2["model_r"]]),3), delta="1st")
...
with col1:
    ...
    doc_1.spans["sc"] = [Span(doc_1, ent.start, ent.end, ent.label_) for ent in doc_1.ents ]
    doc_1.spans["sc"] += [Span(doc_1, ent.start, ent.end, ent.label_) for ent in doc_2.ents]
...
with col2:
    ...
    data['Ents'] = data['Entity'].apply(lambda x: [ent for ent in ents if ent.label_ == x])
