...
# IMPORTANT: See the docstring in `src/infer_entities/extract_entities_local.py` for more details on
...
python src/bulk_inference_pipeline/extract_entities_local.py -p ${PART_OF_PAGE} -m ${NER_MODEL} -d ${YESTERDAY} -c ${CHUNK_SIZE} -b ${BATCH_SIZE} -n ${N_PROC}
