...
def get_strata(content_store_df: pd.DataFrame) -> pd.DataFrame:
    ...
    strata_df = pd.merge(
        content_store_df[["base_path", "schema_name", "document_type", "publishing_app"]],
        taxons,
        how='left',
        on='base_path',
        validate="one_to_one"
...
def filter_level1_taxon(df: pd.DataFrame, id_col: str, taxon_col: str) -> pd.DataFrame:
    ...
    try:
        ...
        exploded_df = exploded_df.applymap(lambda x: REPLACE_VALUE if isinstance(x, float) else x)

        # return level-1 taxon only
        return exploded_df[[isinstance(x, dict) and x['level'] == 1 for x in exploded_df[taxon_col]]]
...
if __name__ == '__main__':
    ...
    print(f"number of unique base_path in content_store_df: {content_store_df.base_path.nunique()}")

    strata_df = get_strata(content_store_df)
    print(f"number of unique base_path in strata_df: {strata_df.base_path.nunique()}")

    STRATA_COUNTS_OUTPUT_FILEPATH = "src/strata/data/strata_counts_schema_docs_pubapp_taxons.csv"
    strata_df.groupby(
        ['publishing_app', 'schema_name', "document_type", 'taxon_level1'])['base_path'].count().to_csv(STRATA_COUNTS_OUTPUT_FILEPATH, index=False)
